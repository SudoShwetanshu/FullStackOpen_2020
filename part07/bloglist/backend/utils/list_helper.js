...
const mostBlogs = (blogs) => {
  const b = _.map(_.countBy(blogs, 'author'), (val, key) => ({ author: key, blogs: val }))
  return _.maxBy(b, 'author')
}

const mostLikes = (blogs) => {
  const reduce = (accumulator, blog) => {
    return !accumulator[blog.author]
      ? { ...accumulator, [blog.author]: blog.likes }
      : { ...accumulator, [blog.author]: accumulator[blog.author] + blog.likes }
  }
  const likes = _.reduce(blogs, reduce, {})

  return _.chain(likes)
    .toPairs()
    .maxBy(_.last)
